...
{
    private HashSet<string> k_SupportedExtensions = new HashSet<string>
    {
        "nn",
        "onnx"
    };
    private const string k_CommandLineModelOverrideDirectoryFlag = "--mlagents-override-model-directory";
    private const string k_CommandLineModelOverrideExtensionFlag = "--mlagents-override-model-extension";
    private const string k_CommandLineQuitAfterEpisodesFlag = "--mlagents-quit-after-episodes";
    private const string k_CommandLineQuitAfterSeconds = "--mlagents-quit-after-seconds";
    private const string k_CommandLineQuitOnLoadFailure = "--mlagents-quit-on-load-failure";
    ...
    private string m_BehaviorNameOverrideDirectory;
    ...
    private int m_PreviousNumSteps;
    private int m_PreviousAgentCompletedEpisodes;

    private bool m_QuitOnLoadFailure;
    ...
    /// </summary>
    /// <returns></returns>
    private void GetAssetPathFromCommandLine()
    {
        ...
        for (var i = 0; i < args.Length; i++)
        {
            ...
            {
                int.TryParse(args[i + 1], out maxEpisodes);
            }
            else if (args[i] == k_CommandLineQuitAfterSeconds && i < args.Length - 1)
            {
                int.TryParse(args[i + 1], out timeoutSeconds);
            }
    ...
    private NNModel LoadBarracudaModel(byte[] rawModel)
    {
    ...
    private NNModel LoadOnnxModel(byte[] rawModel)
    {
